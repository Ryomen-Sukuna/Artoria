...
from tg_bot import (DEV_USERS, OWNER_ID, SUDO_USERS, SUPPORT_CHAT,
...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SUDO_USERS:
        message.reply_text("This member is already a my sudo user")
        return ""

    if user_id in SUPPORT_USERS:
        rt += "promoteing from support user to sudo user."
        data['supports'].remove(user_id)
        SUPPORT_USERS.remove(user_id)

    if user_id in WHITELIST_USERS:
        rt += "promoteing from whitelite user to sudo user."
    ...
    update.effective_message.reply_text(
        rt + "\nSuccessfully set user {} to sudo!".format(
...
def addsupport(
    ...
    if user_id in SUDO_USERS:
        rt += "demoteing sudo user to support user"
        data['sudos'].remove(user_id)
        SUDO_USERS.remove(user_id)

    if user_id in SUPPORT_USERS:
        message.reply_text("This user is already a my support user.")
        return ""

    if user_id in WHITELIST_USERS:
        rt += "promoting user from  whitelist user to support user"
        data['whitelists'].remove(user_id)
        WHITELIST_USERS.remove(user_id)
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in SUDO_USERS:
        message.reply_text("demoteing sudo user to normal user,he deserves it ")
        SUDO_USERS.remove(user_id)
        data['sudos'].remove(user_id)
        ...
        message.reply_text("This user is not a my sudo user!")
        return ""
